...
def status_net():
    ...
    return separator("002b36") + "," + make_json(string, "002b36", "859900")

def status_cpu():
    output = os.popen("conky -i 1").read().rstrip()
    string = "CPU:    " + output.split(", ")[1] + "    "
    return separator("b58900") + "," + make_json(string, "b58900", "002b36")
...
def status_date():
    ...
    string = "   " + day + " " + date_num + "   "
    return separator("859900") + "," + make_json(string, "859900", "002b36")

def status_time():
    time = os.popen("date +'%I:%M:%S'").read().rstrip()
    
    string  = "  " + time + "   "
    return dividor("859900") + "," + make_json(string, "859900", "002b36")
